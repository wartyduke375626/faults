...
\begin{lstlisting}[float,language=AVR,caption={Procedúra oneskorenia v asembleri. \%0 označuje vstupný parameter -- hodnota v registri.},label=alg:asmDelay]
mov r24, %0   ; 1 takt
loop:
    dec r24     ; 1 takt
...
\begin{figure}
    ...
    \caption[Výsledky vylepšeného útoku zo súťaže CTF]{Výsledky vylepšeného útoku zo súťaže CTF. Histogram znázorňuje počet úspešných útokov (z desiatich) na jednotlivé čipy ATMega328P v závislosti od počtu iterácií procedúry oneskorenia z algoritmu \ref{alg:asmDelay}. Každý čip je znázornený rôznou farbou, v legende je uvedená séria príslušného čipu. Vylepšený útok bol zopakovaný desaťkrát pre každý čip a počet iterácií procedúry oneskorenia (1 -- 15), znázornené sú iba hodnoty s nenulovým počtom úspešných útokov. Na čipy zo série 2128BQY pôvodný útok nebol úspešný. Možno pozorovať, že aj čipy z rovnakej série majú rôzne výsledky.}
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \addplot[color=blue, domain=0:10, dotted]
            {3.01*x + 26.68};
        \addplot[color=red, mark=*]
            coordinates {
            (0,19)(1,19)(2,19)(3,20)(4,20)(5,20)(6,21)(7,21)(8,21)(9,21)(10,22)
            };
        \addplot[color=red, domain=0:10, dotted]
            {0.29*x + 18.82};
        \legend {
            Nano,
            $3.01x + 26.68$,
